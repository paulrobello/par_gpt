...
with existing code throughout PAR GPT.
"""
...
def handle_provider_error(provider: str, error: Exception, console: Console | None = None) -> None:
    """Handle AI provider errors with standardized messaging.
    ...
    handler.show_error("AI_PROVIDER_ERROR", provider=provider, error_message=str(error))
...
def handle_config_validation_error(field: str, validation_error: str, console: Console | None = None) -> None:
    ...
    """
    handler = ErrorHandler(console)
    handler.show_error("INPUT_VALIDATION_FAILED", field=field, validation_error=validation_error)
...
def validate_and_handle_errors(
    ...
    try:
        ...
        handler = ErrorHandler(console)
        handler.show_error("INPUT_VALIDATION_FAILED", field="input", validation_error=str(e))
        raise
